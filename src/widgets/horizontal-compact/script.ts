export const HorizontalCompactScript = (index: number) => {
...
const main = () => {
  ...
  const checkinId = `checkin-horizontal-compact-${index}`;
  const checkoutId = `checkout-horizontal-compact-${index}`;
  const guestId = `guest-horizontal-compact-${index}`;
  const shadowRoot = (document.querySelectorAll("[hotelier-widget='horizontal-compact']")[index as number] as HTMLElement).shadowRoot!;
